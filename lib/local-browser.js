...
function normalizeDeviceScaleFactor(value) {
  ...
  const number = Number(value);
  if (!Number.isFinite(number)) return DEFAULT_DEVICE_SCALE_FACTOR;
  return Math.max(DEFAULT_DEVICE_SCALE_FACTOR, Math.min(MAX_DEVICE_SCALE_FACTOR, number));
}
...
function normalizeViewerViewport(value = {}) {
  return {
    ...
    deviceScaleFactor: normalizeDeviceScaleFactor(value.deviceScaleFactor)
  };
}
...
(() => {
  ...
  const point = (event) => {
    ...
    const sourceWidth = Number(visibleFrame?.dataset.viewportWidth) || Math.max(1, rect.width);
    const sourceHeight = Number(visibleFrame?.dataset.viewportHeight) || Math.max(1, rect.height);
  ...
  const refreshFrame = () => {
    ...
    const rect = inputLayer.getBoundingClientRect();
    const width = Math.max(320, Math.min(1920, Math.round(rect.width)));
    const height = Math.max(480, Math.min(1920, Math.round(rect.height)));
    const scale = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
    nextFrame.dataset.viewportWidth = String(width);
    nextFrame.dataset.viewportHeight = String(height);
    nextFrame.src = "/specsrelay/browser/frame?width=" + width +
      "&height=" + height + "&scale=" + scale + "&t=" + Date.now();
  };
